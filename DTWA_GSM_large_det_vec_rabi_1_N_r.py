...
arrRij = np.zeros((2, Natoms, Natoms), complex)
...
for i in range(0, Natoms):
    for j in range(0, Natoms):
        ...
        arrRij[1,i,j] = fac_coh*np.conjugate(evec[1])@np.real(funcGij(i, j))@evec[-1]
...
# arrRij[0] = Rij_{1,1}, arrRij[1] = Rij_{1,-1}

def f_sig_z_dot(sig_z, sig_p): #n = 0, 1, 2, ... = atom no., drive = 0, 1
    #tempsum = -2*sig_z*single_decay    
    tempsum =  - (-4*1j*np.einsum('nj,nj,n,j->n', delta_ij, arrRij[1],sig_p,sig_p)) + (-4*1j*np.einsum('nj,nj,n,j->n', delta_ij, arrRij[1],np.conj(sig_p),np.conj(sig_p)))
...
def f_sig_p_dot(sig_z, sig_p): #n = 0, 1, 2, ... = atom no.
    ...
    tempsum = (2*1j*np.einsum('nj,nj,n,j->n', delta_ij, arrRij[1],sig_z,np.conj(sig_p)))
